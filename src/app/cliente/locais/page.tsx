    ...
    <div className="container mx-auto p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Gerencie os convênios autorizados (bancos, comércios, cooperativas, etc)</p>
        </div>
        {canCreate && (
          <Link
      ...
      <div className="bg-card text-card-foreground rounded-lg shadow-md overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-12 text-muted-foreground">Nenhum convênio encontrado</div>
        ) : (
          <>
            {/* Desktop Header */}
            {!isMobile && (
              <div className="grid grid-cols-[90px_1fr_120px_150px_100px_100px] gap-3 px-4 py-3 bg-muted/50 border-b border-border text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                <div>Código</div>
                    ...
                    >
                      ...
                      <div className="md:hidden p-3 flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="font-medium text-sm text-foreground truncate">{nomePrincipal}</div>
                          {conv.fantasia && conv.fantasia !== nomePrincipal && (
                            <div className="text-xs text-muted-foreground truncate">{conv.fantasia}</div>
                          )}
                      ...
                      <div className="hidden md:grid md:grid-cols-[90px_1fr_120px_150px_100px_100px] gap-3 px-4 items-center h-full text-sm">
                        <div className="font-mono text-xs text-muted-foreground">{conv.codigo || '—'}</div>
                        <div className="min-w-0">
                          <div className="font-medium text-foreground truncate">{nomePrincipal}</div>
                          {conv.fantasia && conv.fantasia !== nomePrincipal && (
