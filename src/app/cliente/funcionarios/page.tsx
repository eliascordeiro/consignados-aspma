  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Sócios</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Gerencie os sócios das consignatárias</p>
        </div>
        {canCreate && (
          <Link
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
        {isLoading ? (
          ...
          <>
            {/* Desktop Header */}
            {!isMobile && (
              <div className="grid grid-cols-[140px_1fr_200px_110px_120px] gap-4 px-4 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600 text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                <div>Matrícula</div>
                <div>Nome</div>
                    ...
                    >
                      ...
                      <div className="md:hidden p-3 flex items-center justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <div className="font-medium text-sm text-gray-900 dark:text-gray-100 truncate">{func.nome}</div>
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${isAtivo ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}`}>
                            {isAtivo ? 'Ativo' : 'Inativo'}
                          </span>
                          {(canView || canEdit) && (
                            <Link href={`/cliente/funcionarios/editar/${func.id}`} className="p-1.5 rounded hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-500 dark:text-gray-400 transition-colors">
                      ...
                      <div className="hidden md:grid md:grid-cols-[140px_1fr_200px_110px_120px] gap-4 px-4 items-center h-full text-sm">
                        <div className="font-mono text-xs text-gray-600 dark:text-gray-300 truncate">{func.matricula || '—'}</div>
                        <div className="font-medium text-gray-900 dark:text-gray-100 truncate">{func.nome}</div>
                        <div className="text-gray-600 dark:text-gray-300 truncate text-xs">{func.empresa?.nome || 'Não informada'}</div>
                        <div>
                          <span className={`text-xs px-2 py-1 rounded-full font-medium ${isAtivo ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}`}>
                            {isAtivo ? 'Ativo' : 'Inativo'}
                          </span>
                        </div>
                        <div className="flex justify-end gap-1">
                          {(canView || canEdit) && (
                            <Link href={`/cliente/funcionarios/editar/${func.id}`} title="Editar" className="p-1.5 rounded hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 transition-colors">
                              <Pencil className="h-4 w-4" />
                            </Link>
                          )}
                          {canDelete && (
