    ...
    <div className="container mx-auto p-6 max-w-4xl">
      ...
      <div className="bg-card text-card-foreground rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-center gap-2">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground">Histórico de Limite</h1>
            {socio && (
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            {/* Header */}
            <div className="grid grid-cols-[150px_140px_160px_160px_1fr] gap-3 px-4 py-3 bg-muted/50 border-b border-border text-xs font-semibold text-muted-foreground uppercase tracking-wider">
              <div>Data</div>
              <div>Usuário</div>
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              {socio.margemHistoricos.map((h) => (
                <div key={h.id} className="grid grid-cols-[150px_140px_160px_160px_1fr] gap-3 px-4 py-3 text-sm hover:bg-gray-100 dark:hover:bg-gray-700 railway:hover:bg-gray-700">
                  <div className="text-xs text-muted-foreground whitespace-nowrap">{formatDate(h.createdAt)}</div>
                  <div className="text-muted-foreground text-xs truncate">{h.usuario.name}</div>
                  <div className="text-right">
                    <div className="flex items-center justify-end gap-1 text-xs font-mono">
                      ...
                      <span className="font-semibold text-foreground">{formatCurrency(h.limiteNovo)}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="flex items-center justify-end gap-1 text-xs font-mono">
                      ...
                      <span className="font-semibold text-foreground">{formatCurrency(h.margemNova)}</span>
                    </div>
                  </div>
                  <div className="min-w-0">
                    <p className="truncate text-muted-foreground" title={h.motivo || ''}>{h.motivo || '—'}</p>
                    {h.observacao && (
