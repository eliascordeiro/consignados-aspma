...
export default function EmpresaAlterarSenhaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/empresa/change-password", {
        method: "POST",
      ...
      if (!res.ok) {
        if (data.error === "conta_bloqueada") setError("HARD_LOCK")
        else setError(data.error || "Erro ao alterar a senha.")
        return
      }
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          <div className="mb-4 flex items-start gap-2 rounded-md border border-amber-500/50 bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-400">
            ...
            <span>
              Defina uma nova senha de acesso ao Portal da Consignatária.
            </span>
          </div>
            ...
            {error === "HARD_LOCK" ? (
              <div className="rounded-md border border-red-500/50 bg-red-500/10 p-4 text-sm">
                ...
                <p className="text-red-600 dark:text-red-300">
                  A senha não foi renovada por mais de 60 dias. Entre em contato com o
                  administrador do sistema para reativar este acesso.
                </p>
              </div>
            ) : error ? (
