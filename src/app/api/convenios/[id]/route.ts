...
export async function PUT(
  ...
  try {
    ...
    if (!newEmail && existing.userId) {
      ...
      // Verificar se o vínculo atual já está correto
      ...
      if (!linkOk) {
        // Localizar ou criar o usuário alvo para este email
        const targetUser = await db.users.findUnique({ where: { email: newEmail } })

        if (targetUser && targetUser.role !== 'USER') {
          // Email pertence a ADMIN/MANAGER — não permitido
          return NextResponse.json(
